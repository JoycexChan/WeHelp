...
// 初始化顧問數據：創建一個數組，其中包含顧問的對象，每個對象具有名字、評分和價格這三個屬性
    ...
    // 時段設定：使用一個對象來映射每個顧問的名字到他們的工作時段。利用 Array.from 來生成每位顧問的24小時可預約時段
        // const 通常用來宣告一個不應該改變的常量
        // Array.from用於從類陣列對象或可迭代對象創建一個新的、淺拷貝的陣列實例。它接受兩個參數
            // 可迭代對象或類陣列對象：用作新陣列元素的數據源
                // { length: 24 }期望生成一個包含 24 個元素的陣列
            // 映射函數（可選）：一個函數，可以對每個元素進行處理，然後返回處理後的值
                // (_, i) => i + 1
                // 這是一個箭頭函數，它接受兩個參數：當前元素的值和當前元素的索引 i
                // _ 是一個佔位符，因爲當前元素的值不存在，所以不關心
                // i 是當前元素的索引，從 0 開始,所以為了獲取24，必須求i + 1
                // 箭頭函數返回 i + 1，這意味著對於陣列中的每個位置，我們實際添加的是索引值加 1 的結果
                // 因此，這個陣列將包含從 1 到 24 的整數。
    const timeDict = {
        John: { addtime: Array.from({ length: 24 }, (_, i) => i + 1) },
        Bob: { addtime: Array.from({ length: 24 }, (_, i) => i + 1) },
        Jenny: { addtime: Array.from({ length: 24 }, (_, i) => i + 1) }
    };
    
    // 遍歷 consultants 數組，並為每個顧問分配對應的時段。這裡使用 forEach 循環，以及 in 運算符來檢查顧問名字是否存在於 timeDict 中
        // forEach用於遍歷陣列的每一個元素，會從 consultants 陣列的第一個元素開始，對每個元素執行在箭頭函數中定義的代碼塊。
        // 箭頭函數：在 forEach 方法中使用了箭頭函數 consultant => { ... }。這個函數對數組中的每個元素（此處為每個顧問）執行內部的代碼塊
        // 首先通過 consultant.name 獲取當前正在處理的顧問的名字，並將其存儲在變量 consultantName 中
        // 使用 in 運算符來檢查變量 consultantName（顧問的名字）是否作為鍵存在於 timeDict 對象中
        // if如果 consultantName 存在於 timeDict 中，則執行任務
        // 將該顧問的 times 屬性設置為從 timeDict 中對應顧問名字的 addtime 屬性獲取的時段數組
    consultants.forEach(consultant => {
        const consultantName = consultant.name;
        if (consultantName in timeDict) {
            consultant.times = timeDict[consultantName].addtime;
        }
    });
    ...
    // 定義預約函數：該函數接受顧問數組、預約的開始時間、持續時間和排序標準作為參數
        ...
        // 使用 filter 和 every 組合來找出所有在 neededTimes 內都有空的顧問
            ...
            // 如果每個時間點都包含，就會都是true，
            // neededTimes.every()只有當每個值都是true的時候才會返回true
    ...
    function book(consultants, start_time, duration, criterion) {
        ...
        const availableConsultants = consultants.filter(consultant =>
            neededTimes.every(time => consultant.times.includes(time))
        );
