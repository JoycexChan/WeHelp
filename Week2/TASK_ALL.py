...
#時段設定，預設所有顧問都是24小時可預約，實際上可能只會開放特定時段
#因此寫入時段字典，拆開其實沒什麼意義，就是題目不能改
    #list是指建立列表，來源為range(1, 25)，當使用range時取得的值僅包含開始值，不包含結束值
    #生成的序列包含从start到stop-1的整數。
time_dict = {
    "John": {"addtime": list(range(1, 25))},
    "Bob": {"addtime": list(range(1, 25))},
    "Jenny": {"addtime": list(range(1, 25))}
}

# 從consultants字典中叫出所有值稱之consultant
    # 設置consultant_name變數，來源為consultant中的name
    # 如果consultant_name變數代稱值，包含在time_dict字典中，注意要加:說明如果if為真，會發生什麼事情
    # consultant中會添加一個times的Key，內容物為time_dict字典中包含consultant_name key的addtime
for consultant in consultants:
    consultant_name = consultant["name"]
    if consultant_name in time_dict:
        consultant["times"] = time_dict[consultant_name]["addtime"]
...
def book(consultants, start_time, duration, criterion):
    ...
    # 從consultants字典中提出所有值，命名為consultant
        # if為真的時候，執行:後的任務
        ...
        # 因此if needed_times使用者期待的值(time為局部變量，在上下文一致即可，可以改名)
        # 若符合consultants字典的值(time為局部變量)中，times key中的值
        # 執行:後的任務
            # 從consultants字典裡的值consultant，提取符合的顧問，以.append添加到available_consultants
    available_consultants = []
    for  consultant in consultants:
        if all(time in consultant['times'] for time in needed_times):
            available_consultants.append(consultant)

    # 選擇顧問
    # 如果available_consultants列表不存在，則印出No Service，並且return終止函數
...
## 整理函數處理後的資料並輸出使用者目標答案：
    ...
    # 接著同步更新到最完整的consultants列表（全部的顧問字典）
    # consultants列表中的值稱之consultant
    # 如果consultant裡面的key name下的名字與hosen_consultant key name下的名字相符合，則執行任務：
    # 將consultant裡面的key times下的時間替換為hosen_consultant key times下的時間
    # 這樣預約後才會把該時段刪除
    for consultant in consultants:
        if consultant['name'] == chosen_consultant['name']:
            consultant['times'] = chosen_consultant['times']

    # 以目標格式列印
        # 如果想要添加額外的文字可以用
        # print(f"成功預約 {chosen_consultant['name']}")
        # f指的是 f-string，利用這個方式可以嵌入變量跟表達式
        # 如果沒有加f而使用文字跟變量或表達式，就會視為文字直接印出
